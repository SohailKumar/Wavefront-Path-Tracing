...
//
//    dim3 blockSize = dim3(16, 16); // block dimensions are fixed to be 256 threads
//    dim3 gridSize = dim3((width + blockSize.x - 1) / blockSize.x, (height + blockSize.y - 1) / blockSize.y);
//
//    cuda_kernel_texture_2d <<<gridSize, blockSize>>> ((unsigned char*)surface, width, height, pitch, t);
//
